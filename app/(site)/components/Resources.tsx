    ...
    <section id="resources" className="py-20 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          {items.map((r) => (
            <div key={r.title} className="bg-white rounded-2xl p-6 border border-neutral-100 hover:shadow-sm">
              <div className="flex items-start justify-between gap-4 mb-3">
                ...
                <button className="px-4 py-2 rounded-lg bg-[color:rgb(var(--brand-50))] text-[color:rgb(var(--brand-700))]">{r.size}</button>
              </div>
              <div className="flex flex-wrap gap-2">
                {r.chips.map((c) => (
                  <span key={c} className="text-xs bg-neutral-50 px-3 py-1 rounded-md border border-neutral-200">{c}</span>
                ))}
              </div>
            </div>
          ))}
